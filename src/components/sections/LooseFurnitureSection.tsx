...
import { LooseFurnitureItem, ClientProject } from '../../types';
import {
  User,
  FileText,
  Globe,
...
interface LooseFurnitureSectionProps {
  items: LooseFurnitureItem[];
...
export const LooseFurnitureSection: React.FC<LooseFurnitureSectionProps> = ({
  items,
  clients = [],
  client,
  showAllClients = true,
            ...
            <div className="flex items-center space-x-2">
              ...
              {onRefresh && (
                <button
                  ...
                  title="Refresh Loose Furniture list"
                >
        ...
        {dataMode === 'clientWise' ? (
          ...
          <div className="bg-slate-100/80 border border-slate-200/80 rounded-lg p-2 px-3 text-[11px] text-slate-600 flex items-center justify-between">
            <span className="flex items-center space-x-1.5">
              ...
              <span>Viewing combined loose furniture records across <strong>all CRM clients</strong>.</span>
            </span>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-md text-white text-[10px] font-extrabold uppercase shadow-xs shrink-0 flex items-center space-x-1 ${
                      statusLower === 'recieved' || statusLower === 'approved'
                        ? 'bg-emerald-600'
                        : 'bg-amber-500'
                    }`}
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-2.5 border-t border-slate-200/80">
                  <div>
                    <span className="text-slate-400 block text-[10px] font-semibold">File / Item Name</span>
                ...
                {/* Vendor Upload URL if available */}
                {item.upload_url && item.upload_url !== 'test' && (
                  <div className="bg-white p-2 rounded-lg border border-slate-200 flex items-center justify-between text-xs">
                    <span className="text-[10px] font-semibold text-slate-400 uppercase">Upload / Design Link</span>
                    <button
                      onClick={() => openUrl(item.upload_url)}
                      className="text-sky-600 font-medium hover:underline text-xs flex items-center space-x-1 truncate max-w-[280px] cursor-pointer"
                    >
                      <span className="truncate">{item.upload_url}</span>
                ...
                <div className="pt-2 border-t border-slate-200/80 flex flex-wrap items-center justify-between gap-2">
                  <div className="text-[10px] text-slate-400 italic">
                    Rec ID: <span className="font-mono text-slate-600 font-semibold">{item.id}</span>
                  </div>
